...
macro_rules! parse_hip_field {
    ...
    (ra_hms, $s:expr) => {{
        ...
        RightAscension(Angle::from(Hms::new(
            Sign::Plus,
            parse_hip_field!(u32, fields[0]).unwrap(),
            parse_hip_field!(u32, fields[1]).unwrap(),
            parse_hip_field!(f64, fields[2]).unwrap(),
        )))
    }};
    (dec_dms, $s:expr) => {{
        let fields: Vec<&str> = $s.split(" ").collect();
        ...
        Declination(Angle::from(Dms::new(
            sign,
            parse_hip_field!(u32, fields[0][1..]).unwrap(),
            parse_hip_field!(u32, fields[1]).unwrap(),
            parse_hip_field!(f64, fields[2]).unwrap(),
        )))
    }};
...
mod tests {
    ...
    fn test_hipparse() {
        ...
        println!("{}", s);
        let _star = HipStar::try_from(s);
        println!("{:?}", _star);
    }

    #[test]
    fn test_catalog() {
        let _stars = parse_catalog!(HipStar, Path::new("data/Hipparcos/hip_main.dat"), None);
